...
class Intersection:
    ...
    def update_neighbor_info(self, neighbors, dic_feature):
        ...
        for i in range(len(neighbors)):
            ...
            if neighbor is None:
                ...
                # example_dic_feature["lane_num_vehicle_been_stopped_thres1_{0}".format(i)]= none_dic_feature["lane_num_vehicle_been_stopped_thres1"]
            else:
                ...
                example_dic_feature["lane_num_vehicle_{0}".format(i)] = [x*0.9 for x in neighbor.dic_feature["lane_num_vehicle"]]
                # example_dic_feature["lane_num_vehicle_been_stopped_thres1_{0}".format(i)]= neighbor.dic_feature["lane_num_vehicle_been_stopped_thres1"]
    ...
    def _update_feature_map(self, simulator_state):
        ...
        dic_feature["pressure"] = None  #None  # [self._get_pressure()]
        ...
        # dic_feature["lane_num_vehicle_been_stopped_thres1"] = None
        dic_feature["lane_num_vehicle_been_stopped_thres1"] = self._get_lane_num_vehicle_been_stopped(self.list_entering_lanes)
        ...
        dic_feature["pressure"]=None

        # dic_feature[
        #     "adjacency_matrix"] = self._get_adjacency_row()
        dic_feature[
            "adjacency_matrix"] = None  # TODO this feature should be a dict? or list of lists

        dic_feature[
            "adjacency_matrix_lane"] = None # row: entering_lane # columns: [inputlanes, outputlanes]
            # "adjacency_matrix_lane"] = self._get_adjacency_row_lane()  # row: entering_lane # columns: [inputlanes, outputlanes]

        dic_feature['connectivity'] =None
    ...
    def _get_lane_num_vehicle_been_stopped(self,  list_lanes):
        return [self.dic_lane_waiting_vehicle_count_current_step[lane] for lane in list_lanes]
    ...
    def get_reward(self, dic_reward_info):
        ...
        # dic_reward["sum_num_vehicle_been_stopped_thres1"] = None

        dic_reward['pressure'] = np.sum(self.dic_feature["pressure"])    #None
                                 # np.sum(self.dic_feature["pressure"])
        #print(np.sum(
        #    self.dic_feature["lane_num_vehicle_been_stopped_thres1"]), "rere")
        reward = 0
...
class AnonEnv:
    ...
    def __init__(self, path_to_log, path_to_work_directory, dic_traffic_env_conf):
        ...
        self.agent_num = self.dic_traffic_env_conf["NUM_INTERSECTIONS"]
    ...
    def reset(self):
        ...
        #    for j in range(len(lane_num_vehicle)):
        #        a += lane_num_vehicle[i]
        #    print(a, "lan")

        return state

    #log_model=False是否 保存日志
    def step(self, action,log_model=False):
        ...
        #reward,in_reward = self.get_reward()
        reward,in_reward = self.get_reward_new()


        # print("Step time: ", time.time() - step_start_time)
        return next_state, reward, done,in_reward


    def step_ano(self, action,log_model=False):
        # step_start_time = time.time()
        list_action_in_sec = [action]
        list_action_in_sec_display = [action]
        for i in range(self.dic_traffic_env_conf["MIN_ACTION_TIME"] - 1):
    ...
    def step_pearson(self, action,log_model=False):
        # step_start_time = time.time()
        list_action_in_sec = [action]
        list_action_in_sec_display = [action]
        for i in range(self.dic_traffic_env_conf["MIN_ACTION_TIME"] - 1):
        ...
        #print(next_state, "next_state")
        #reward,in_reward = self.get_reward()
        reward,in_reward,nei_reward = self.get_reward_pearson()


        # print("Step time: ", time.time() - step_start_time)
        return next_state, in_reward, done,nei_reward
    ...
    def get_reward(self):
        ...
        in_reward=deepcopy(list_reward)
        if self.dic_traffic_env_conf["NEIGHBOR"]:
            for i,inter in enumerate(self.list_intersection):
                ...
                for neighbor_inter_id in neighbor_inter_ids:
                    if neighbor_inter_id is not None:
                        neighbor_inters.append(self.list_intersection[self.id_to_index[neighbor_inter_id]])
                for neighbor_inter in neighbor_inters:
                    list_reward[i]+=neighbor_inter.get_reward((self.dic_traffic_env_conf["DIC_REWARD_INFO"]))
                list_reward[i]=float(list_reward[i]/(len(neighbor_inters)+1))

        return list_reward,in_reward

    def get_reward_simple(self):
        ind_reward = [inter.get_reward(self.dic_traffic_env_conf["DIC_REWARD_INFO"])  for inter in
                       self.list_intersection]
        return ind_reward
        ...
        if self.dic_traffic_env_conf["NEIGHBOR"]:
            for i,inter in enumerate(self.list_intersection):
                ...
                for neighbor_inter in neighbor_inters:
                    ...
                    a = neighbor_inter.get_reward((self.dic_traffic_env_conf["DIC_REWARD_INFO"]))
                ...
                list_reward[i]=float(list_reward[i]/(t+1))

        return list_reward,in_reward


    def get_reward_pearson(self):
        list_reward = [inter.get_reward(self.dic_traffic_env_conf["DIC_REWARD_INFO"])  for inter in
                       self.list_intersection]
            ...
            for i,inter in enumerate(self.list_intersection):
                ...
                for neighbor_inter in neighbor_inters:
                    ...
                    if neighbor_inter != 0:
                        ...
                        nei_reward_list.append(neighbor_inter.get_reward((self.dic_traffic_env_conf["DIC_REWARD_INFO"])))

                    else:
                        nei_reward_list.append(99)

                t = 0
                for i in range(4):
                    if neighbor_inters[i] == 0:
                        t +=1
                if t >0:
                    neighbor_inters.remove(0)


                nei_reward_lists.append(nei_reward_list)


        return list_reward,in_reward,nei_reward_lists
        ...
        with open('{0}'.format(file)) as json_data:
            ...
            for i in traffic_light_node_dict.keys():
                ...
                if not self.dic_traffic_env_conf['ADJACENCY_BY_CONNECTION_OR_GEO']:  # use geo-distance
                    ...
                    if len(row) == top_k:
                        adjacency_row_unsorted = np.argpartition(row, -1)[:top_k].tolist()
                        # np.argpartition对于传入的数组a，先用O(n)复杂度求出第k大的数字，然后利用这个第k大的数字将数组a划分成两半。
                    elif len(row) > top_k:
                        adjacency_row_unsorted = np.argpartition(row, top_k)[:top_k].tolist()
                    else:
                        adjacency_row_unsorted = [k for k in range(total_inter_num)]
...
if __name__ == '__main__':
    ...
    dic_traffic_env_conf = {
        ...
        "TRAFFIC_FILE": "anon_6_6_300_0.3_bi.json",
        "THREADNUM": 8,
        "SAVEREPLAY": False,
        "RLTRAFFICLIGHT": True,
        "INTERVAL": 1,
        "NUM_INTERSECTIONS": 36,
        "ACTION_PATTERN": "set",
        "MEASURE_TIME": 10,
        "MIN_ACTION_TIME": 10,
        "YELLOW_TIME": 5,
        ...
        "VOLUME": 300,
        "ROADNET_FILE": "roadnet_{0}.json".format("6_6"),
    ...
    dic_path = {
        ...
        "PATH_TO_WORK_DIRECTORY": "records/test",
